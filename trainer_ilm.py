...
def masked_dataset_to_inputs_and_tts(
  ...
  if split == 'train':
    examples_tag = train_examples_tag
    sequence_length = train_sequence_length
    max_num_examples = train_max_num_examples
    skip_naive_incomplete = train_skip_naive_incomplete
  else:
    examples_tag = eval_examples_tag
    sequence_length = eval_sequence_length
    max_num_examples = eval_max_num_examples
    skip_naive_incomplete = eval_skip_naive_incomplete

  with open(os.path.join(examples_dir, '{}.pkl'.format(examples_tag)), 'rb') as f:
  ...
  _GLOBAL_WORKER_TARGET = worker_target_factory(
      ...
      Task[task.upper()],
      skip_naive_incomplete)
  with multiprocessing.Pool(data_loader_num_workers) as p:
...
def train(experiment_name='custom',
train_dir='train', 
examples_dir='/data/char_masks/custom',
seed=0,
wandb=False,
...
eval_skip_naive_incomplete=False):
  ...
  example_tag_to_fp = lambda tag: os.path.join(examples_dir, '{}.pkl'.format(tag))
  out_fn_to_fp = lambda fn: os.path.join(train_dir, fn)
  ...
  if train_num_epochs is not None:
    train_num_batches = int(float(train_num_docs * train_num_epochs) / train_batch_size)
    if train_num_batches == 0:
      return
    print('Maximum number of training steps: {}'.format(train_num_batches / train_batch_accumulation))
  ...
  set_random_seed(seed)
  if model_name in ilm.constants.GPT2_MODEL_NAMES:
    model_type = GPT2LMHeadModel
    cfg_type = GPT2Config
  if resuming:
    print('from saved checkpoint (resuming)')
    model = model_type.from_pretrained(train_dir)
  else:
    if train_from_scratch:
      print('from scratch')
      cfg = cfg_type.from_pretrained(model_name)
  ...
  if eval_only:
    ...
    num_save = -1
    num_summary = -1
    num_batches_complete = step * train_batch_accumulation
    start = time.time()
    while True:
      if train_num_epochs is not None and num_batches_complete >= train_num_batches:
        break
      ...
      for batch in train_dataloader:
        ...
        # Evaluate
        if int(elapsed / train_eval_secs) > num_save:
          num_save = int(elapsed / train_eval_secs)
        ...
        if train_batch_accumulation != 1:
          loss /= float(train_batch_accumulation)
        loss.backward()

        # Summarize
        if int(elapsed / train_summary_secs) > num_summary:
          num_summary = int(elapsed / train_summary_secs)
        ...
        if ((num_batches_complete + 1) % train_batch_accumulation) == 0:
